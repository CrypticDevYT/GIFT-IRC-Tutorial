...
func _ready() -> void:
	...
	authfile.open("res://config.json", File.READ)
	var dict = parse_json(authfile.get_as_text())
	authfile.close()
	connect_to_twitch()
	...
	authenticate_oauth(dict.botname, dict.token)
	if(yield(self, "login_attempt") == false):
	  print("Invalid username or token.")
	  return
	join_channel(dict.initial_channel)

	connect("cmd_no_permission", self, "no_permission")
	connect("chat_message", self, "print_chat_message")
func print_chat_message(sender_data: SenderData, message: String):
	print(sender_data.user + " said " + message)
